...
import { DISCLAIMER, DEFAULT_BOOKING_URL, DEFAULT_CONTACT_PHONE, DEFAULT_CONTACT_MESSAGE } from "../config/defaults";
...
export default function Results() {
  ...
  async function sendEmail() {
    setStatus("");
    if (!consent) { setStatus("Zaznacz zgodę na udostępnienie wyników."); return; }
    setSending(true);
    try {
      const payload = {
        consent: true,
        results: { topSchemas: top.map(t => ({ name: t.name, level: t.level, score: t.score })), note: note.trim() },
        meta: { ageGroup }
      };
  ...
  return (
    <div className="mx-auto max-w-md px-4 pt-6 pb-20">
      ...
      <div className="mt-2 text-xs text-slate-300/90 border border-slate-800 rounded-xl p-3 bg-slate-900/40">{DISCLAIMER}</div>
      ...
      <div className="mt-5 rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <div className="mt-2 grid gap-2">
          {top.map(t => (
            <div key={t.id} className="p-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10">
              <div className="text-sm font-semibold">{t.name}</div>
              <div className="text-xs text-slate-200/80">{t.domain}</div>
              <div className="mt-1 text-xs text-slate-200/80">Poziom: <span className="font-semibold">{t.level}</span> • score: {t.score}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
        ...
        <div className="mt-3 grid gap-3">
          <Tile title="Porozmawiaj o wyniku (AI)" subtitle="Pytania prowadzone + odpowiedzi dopasowane do wyniku." onClick={() => nav("/pytania")} tone="rose" />
          <Tile title="Pobierz PDF" subtitle="Zapisz wynik lokalnie (bez wysyłki)." onClick={downloadPdf} tone="sky" />
        </div>
      </div>
  ...
  <Tile title="Umów wizytę" subtitle="Przejdź do rejestracji (ZnanyLekarz)." onClick={() => window.open(bookingUrl, "_blank")}
    tone="emerald" right={<span className="text-xs">↗</span>} />
  <Tile title="WhatsApp" subtitle="Napisz do terapeuty z gotowym szablonem." onClick={() => window.open(buildWhatsAppUrl(phone, DEFAULT_CONTACT_MESSAGE), "_blank")}
    tone="emerald" right={<span className="text-xs">↗</span>} />
  ...
  <Tile title="Zadzwoń" subtitle="Połączenie telefoniczne." onClick={() => { window.location.href = buildTelUrl(phone); }}
    tone="orange" />
  <button onClick={() => nav("/test")} className="w-full py-3 rounded-xl border border-slate-700 bg-slate-900/40">Wróć do testu</button>
</div>
      </div>
    </div>
  );
